...
namespace SS_SuizaNaval_API.Infra.Data
{
    public class BaseHttpClient
    {
        ...
        protected async Task<List<T>> GetList<T>(string uri)
        {
            var request = CreateRequestList(HttpMethod.Get, uri);

            return await ExecuteRequestList<T>(request);
        }
        ...
        private static HttpRequestMessage CreateRequestList(HttpMethod httpMethod, string uri, object content = null)
        {
            var request = new HttpRequestMessage(httpMethod, uri);
            if (content == null) return request;
        ...
        private async Task<List<T>> ExecuteRequestList<T>(HttpRequestMessage request)
        {
            try
            {
                var response = await _httpClient.SendAsync(request, HttpCompletionOption.ResponseHeadersRead).ConfigureAwait(false);
                ...
                return string.IsNullOrEmpty(responseContent) ? default : JsonSerializer.Deserialize<List<T>>(responseContent);
            }
            catch (Exception ex) when (ex is ArgumentNullException ||
                                       ex is InvalidOperationException ||
                                       ex is HttpRequestException ||
